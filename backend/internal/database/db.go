...
var appLogger = slog.New(slog.NewJSONHandler(os.Stdout, &slog.HandlerOptions{
...
func Connect() {
	...
	for i := 1; i <= 10; i++ {
		...
		appLogger.Error("database_connection_retry",
			slog.Int("attempt", i),
	...
	if err != nil {
		appLogger.Error("database_connection_failed", slog.String("error", err.Error()))
		os.Exit(1)
	}
	if err := DB.Use(gormtracing.NewPlugin()); err != nil {
		appLogger.Error("database_tracing_plugin_failed", slog.String("error", err.Error()))
		os.Exit(1)
	}
	...
	sqlDB, sqlErr = DB.DB()
	if sqlErr != nil {
		appLogger.Error("database_pool_unavailable", slog.String("error", sqlErr.Error()))
		os.Exit(1)
	}
	...
	if err != nil {
		appLogger.Error("database_schema_update_failed", slog.String("error", err.Error()))
		os.Exit(1)
	}

	appLogger.Info("database_ready")
	seedProducts()
}
...
func seedProducts() {
	...
	DB.Create(&products)
	appLogger.Info("database_seeded", slog.Int("product_count", len(products)))
}
